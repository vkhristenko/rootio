#ifndef rootio_core_localfs_h
#define rootio_core_localfs_h
...
void                          localfs_write_to_file(struct localfs_file_t, 
                                    char const*, int);
void                          localfs_write_to_file_at(struct localfs_file_t, 
                                    long, char const*, int);
...
void                          localfs_close_from_write(
                                    struct localfs_file_context_t*);

//
// api: private functions, modify the state
//
void                          localfs_read_file_header(
                                    struct localfs_file_context_t*);
void                          localfs_read_top_dir_record(
                                    struct localfs_file_context_t*);

//
// public functions, no state modifications
//
struct streamer_record_t      localfs_read_streamer_record(struct 
                                    localfs_file_context_t*);
struct free_segments_record_t localfs_read_free_segments_record(struct 
                                    localfs_file_context_t*);
struct directory_record_t     localfs_read_dir_record_by_key(
                                    struct localfs_file_context_t const*, 
                                    struct key_t const*);
struct generic_record_t       localfs_read_generic_record_at(
                                    struct localfs_file_context_t const*, 
                                    uint64_t);
struct keys_list_record_t     localfs_read_keys_list_record_for_dir(
                                    struct localfs_file_context_t const*, 
...
void                          localfs_write_file_header(
                                    struct localfs_file_context_t*);
void                          localfs_write_top_dir_record(
                                    struct localfs_file_context_t*);
